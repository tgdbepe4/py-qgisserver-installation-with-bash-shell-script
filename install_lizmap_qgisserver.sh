...
# Must be up before any QGIS Server worker starts
Before=qgis-server@1.socket qgis-server@2.socket qgis-server@3.socket qgis-server@4.socket
...
QGIS_SOCKET_UNITS=""
for i in $(seq 1 "${QGIS_WORKER_COUNT}"); do
    QGIS_SOCKET_UNITS+=" qgis-server@${i}.socket"
done

cat > /etc/systemd/system/qgis.service <<UNIT
[Unit]
Description=QGIS Server Stack (Xvfb + FastCGI workers + py-qgis-server)
Documentation=https://docs.lizmap.com
After=network.target postgresql.service
Wants=xvfb.service${QGIS_SOCKET_UNITS} supervisor.service

[Service]
Type=oneshot
RemainAfterExit=yes

ExecStart=/bin/bash -c 'systemctl start xvfb.service${QGIS_SOCKET_UNITS} supervisor.service && supervisorctl start py-qgisserver'
ExecStop=/bin/bash  -c 'supervisorctl stop py-qgisserver; systemctl stop${QGIS_SOCKET_UNITS} xvfb.service'
ExecReload=/bin/bash -c 'systemctl restart xvfb.service${QGIS_SOCKET_UNITS}; supervisorctl restart py-qgisserver'
...
server {
    ...
    # ---- Lizmap Web Client --------------------------------------------------
    # Web root = lizmap/www/ (index.php lives here, NOT in the project root)
    root ${LIZMAP_DIR}/lizmap/www;
    index index.php;
    ...
    # Lizmap 3.9+ requires py-qgis-server. It listens on port 7200.
    # /ows/    — OGC requests (WMS, WFS, WCS, WMTS)
    # /lizmap/ — Lizmap plugin API (GetFeatureInfo extensions etc.)
...
if [ -z "${CERTBOT_EMAIL}" ]; then
    ...
    if certbot --nginx -d "${CERTBOT_DOMAIN}" \
        ...
        # Certbot setzt 'return 404' für Nicht-Domain-Anfragen (z.B. direkte IP-Zugriffe).
        # Fix: alle HTTP-Anfragen auf die HTTPS-Domain weiterleiten statt 404 zurückzugeben.
        sed -i "s|return 404;.*Certbot|return 301 https://${CERTBOT_DOMAIN}\$request_uri;|" \
            /etc/nginx/sites-available/lizmap
        nginx -t && systemctl reload nginx
        log "Nginx: IP access now redirects to https://${CERTBOT_DOMAIN}"
    else
        ...
        warn "Danach IP-Redirect fixen:"
        warn "  sed -i 's|return 404;.*Certbot|return 301 https://${CERTBOT_DOMAIN}\$request_uri;|' /etc/nginx/sites-available/lizmap"
